...
export function ForgotPasswordCard({
  ...
  const onSubmit: SubmitHandler<SendResetPasswordInstructionsSchema> = async (
    ...
    if (!result?.serverError && !result?.validationErrors) {
      ...
      setErrorMessage(t('forgotPassword.errorMessage') || "Couldn't request password change");
    }
  };
  return (
    <Card
    ...
    >
      <CardHeader>
        ...
        <CardDescription>
          {t('forgotPassword.description') || "No worries! We'll send you a link with instructions on how to reset your password."}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <FormProvider {...methods}>
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  <FormLabel>{t('forgotPassword.email') || 'Email'}</FormLabel>
                  <FormControl>
                    <InputWithAdornments
                      {...field}
                      type="email"
            ...
            >
              {t('forgotPassword.sendInstructions') || 'Send instructions'}
            </Button>
          </form>
        </FormProvider>
      </CardContent>
      <CardFooter className="flex justify-center gap-1 text-sm text-muted-foreground">
        <span>{t('forgotPassword.rememberedPassword') || 'Remembered your password?'}</span>
        <Link
