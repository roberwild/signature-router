...
function ConnectedAccountListItem({
  ...
  const handleConnect = async () => {
    ...
    if (result?.serverError || result?.validationErrors) {
      toast.error(t('security.connectedAccounts.messages.connectError'));
    }
  };
  const handleDisconnect = async () => {
    ...
    if (!result?.serverError && !result?.validationErrors) {
      toast.success(t('security.connectedAccounts.messages.disconnected'));
    } else {
      toast.error(t('security.connectedAccounts.messages.disconnectError'));
    }
  };
  return (
    <li
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {connectedAccount.linked
              ? t('security.connectedAccounts.status.connected')
              : t('security.connectedAccounts.status.disconnected')}
          </p>
        </div>
      </div>
      {connectedAccount.linked ? (
        ...
        >
          {t('security.connectedAccounts.actions.disconnect')}
        </Button>
      ) : (
        <Button
          type="button"
        ...
        >
          {t('security.connectedAccounts.actions.connect')}
        </Button>
      )}
    </li>
  );
