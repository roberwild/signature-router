...
function AuthenticatorAppListItem({
  ...
  const handleShowEnableAuthenticatorAppModal = async (): Promise<void> => {
    ...
    if (result?.data) {
      ...
      toast.error(t('security.multiFactorAuth.messages.setupError'));
    }
  };
      ...
      <div className="flex min-w-0 flex-row items-center gap-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <h5 className="overflow-hidden truncate text-sm font-medium">
            {t('security.multiFactorAuth.title')}
          </h5>
          <div className="overflow-hidden truncate text-sm text-muted-foreground">
            {isEnabled ? t('security.multiFactorAuth.status.enabled') : t('security.multiFactorAuth.status.disabled')}
          </div>
        </div>
      </div>
      {isEnabled ? (
        ...
        >
          {t('security.multiFactorAuth.actions.disable')}
        </Button>
      ) : (
        <Button
          type="button"
        ...
        >
          {t('security.multiFactorAuth.actions.enable')}
        </Button>
      )}
    </li>
  );
