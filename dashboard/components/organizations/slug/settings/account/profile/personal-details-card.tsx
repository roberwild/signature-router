...
export function PersonalDetailsCard({
  ...
  const handleDrop = async (files: File[]): Promise<void> => {
    if (files && files.length > 0) {
      ...
      if (file.size > MAX_IMAGE_SIZE) {
        toast.error(
          t('profile.personalDetails.messages.imageSizeError', { size: MAX_IMAGE_SIZE / 1000000 })
        );
      } else {
  ...
  const onSubmit: SubmitHandler<UpdatePersonalDetailsSchema> = async (
    ...
    if (!result?.serverError && !result?.validationErrors) {
      toast.success(t('profile.personalDetails.messages.updateSuccess'));
    } else {
      toast.error(t('profile.personalDetails.messages.updateError'));
    }
  };
  return (
    <FormProvider {...methods}>
      <Card {...other}>
          ...
          >
              ...
              <div className="relative">
                ...
                {image && (
                  <Tooltip>
                    ...
                    <TooltipContent side="right">{t('profile.personalDetails.actions.removeImage')}</TooltipContent>
                  </Tooltip>
                )}
              </div>
            </div>
            <div className="grid gap-x-8 gap-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel required>{t('profile.personalDetails.fields.name')}</FormLabel>
                    <FormControl>
                      <Input
                        type="text"
                        maxLength={32}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('profile.personalDetails.fields.phone')}</FormLabel>
                    <FormControl>
                      <Input
                        type="tel"
                        maxLength={32}
              ...
              <div className="space-y-2">
                <FormLabel>{t('profile.personalDetails.fields.email')}</FormLabel>
                <InputWithAdornments
                  type="email"
                  maxLength={255}
                  ...
                  endAdornment={
                    ...
                    >
                      {t('profile.personalDetails.actions.changeEmail')}
                    </Button>
                  }
                  disabled
                />
        ...
        <CardFooter className="flex w-full justify-end pt-6">
          ...
          >
            {t('profile.personalDetails.actions.save')}
          </Button>
        </CardFooter>
      </Card>
    </FormProvider>
