...
export const CreateApiKeyModal = NiceModal.create<CreateApiKeyModalProps>(
  () => {
    ...
    const title = t('developers.apiKeys.actions.create');
    const description = t('developers.apiKeys.messages.createDescription');
    ...
    const onSubmit: SubmitHandler<CreateApiKeySchema> = async (values) => {
      ...
      if (
        ...
        toast.success(t('developers.apiKeys.messages.createSuccess'));
        modal.resolve(result.data.apiKey);
        modal.handleClose();
      } else {
        toast.error(t('developers.apiKeys.messages.createError'));
      }
    };
    const renderForm = (
      <form
      ...
      >
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex w-full flex-col">
              <FormLabel required>{t('developers.apiKeys.fields.description')}</FormLabel>
              <FormControl>
                <Input
                  type="text"
                  required
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex flex-row items-center justify-between">
            <FormLabel required>{t('developers.apiKeys.fields.expires')}</FormLabel>
            <FormField
              control={methods.control}
              name="neverExpires"
              render={({ field }) => (
                <FormItem>
                  <div className="flex flex-row items-center gap-1">
                    ...
                    <FormLabel className="leading-2 cursor-pointer">
                      {t('developers.apiKeys.status.neverExpires')}
                    </FormLabel>
                  </div>
                </FormItem>
              )}
                    ...
                    <FormControl>
                      ...
                      >
                        {field.value ? (
                          ...
                          <span>{t('developers.apiKeys.fields.pickDate')}</span>
                        )}
...
{t('developers.apiKeys.actions.cancel')}
        ...
        >
{t('developers.apiKeys.actions.createButton')}
        </Button>
      </>
    );
    return (
