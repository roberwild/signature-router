...
export const EditInvitationModal = NiceModal.create<EditInvitationModalProps>(
  ({ profile, invitation }) => {
    ...
    const title = t('members.invitations.messages.updateInvitationTitle');
    const description = t('members.invitations.messages.updateInvitationDescription');
    const canSubmit =
    ...
    const onSubmit: SubmitHandler<UpdateInvitationSchema> = async (values) => {
      ...
      if (!result?.serverError && !result?.validationErrors) {
        toast.success(t('members.invitations.messages.inviteUpdateSuccess'));
        modal.handleClose();
      } else {
        toast.error(t('members.invitations.messages.inviteUpdateError'));
      }
    };
    const renderForm = (
      <form
