...
{t('members.invitations.status.pending')}
              </TabsTrigger>
              <TabsTrigger value={InvitationStatus.REVOKED}>
                {t('members.invitations.messages.revoked')}
              </TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
      ...
      <CardContent className="max-h-72 flex-1 overflow-hidden p-0">
        {filteredInvitations.length > 0 ? (
          ...
          <EmptyText className="p-6">
            {searchQuery
              ? t('members.invitations.messages.noInvitationsSearch')
              : t('members.invitations.messages.noInvitations')}
          </EmptyText>
        )}
      </CardContent>
    </Card>
