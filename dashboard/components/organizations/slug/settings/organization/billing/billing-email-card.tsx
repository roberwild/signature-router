...
export function BillingEmailCard({
  ...
  const onSubmit: SubmitHandler<UpdateBillingEmailSchema> = async (values) => {
    ...
    if (!result?.serverError && !result?.validationErrors) {
      toast.success(t('billing.billingEmail.messages.updateSuccess'));
    } else {
      toast.error(t('billing.billingEmail.messages.updateError'));
    }
  };
  return (
    <FormProvider {...methods}>
      <Card {...other}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex w-full flex-col">
                  <FormLabel>{t('billing.billingEmail.fields.email')}</FormLabel>
                  <FormControl>
                    <Input
                      type="email"
                      maxLength={255}
        ...
        <CardFooter className="flex w-full justify-end pt-6">
          ...
          >
            {t('billing.billingEmail.actions.save')}
          </Button>
        </CardFooter>
      </Card>
    </FormProvider>
