...
export function SubscriptionPlanDetails({
  ...
  const handleUpgrade = async (): Promise<void> => {
    ...
    if (result?.data?.session?.id) {
      ...
      toast.error(t('billing.subscriptionPlan.messages.checkoutError'));
    }
  };
  ...
  const handleBillingPortalRedirect = async (): Promise<void> => {
    ...
    if (result?.data?.url) {
      ...
      toast.error(t('billing.subscriptionPlan.messages.billingPortalError'));
    }
  };
  ...
  const handleBillingRedirect = async (): Promise<void> => {
    ...
    try {
      ...
      toast.error(t('billing.subscriptionPlan.messages.unexpectedError'));
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <div>
        <p className="text-sm text-muted-foreground">
          {t('billing.subscriptionPlan.currentPlanMessage')}
        </p>
        <p>
          <span className="text-2xl">{plan.displayName}</span>
          {plan.isCanceled && (
            <span className="ml-2 text-sm text-muted-foreground">
              ({t('billing.subscriptionPlan.status.canceled')})
            </span>
          )}
        </p>
      </div>
      <div>
        ...
        >
          {isFreePlan ? t('billing.subscriptionPlan.actions.upgradeToPro') : t('billing.subscriptionPlan.actions.changeSubscription')}
        </Button>
      </div>
      {!isFreePlan && (
        <div className="flex w-full flex-col">
          <div className="flex justify-between space-x-8 pb-1 align-baseline">
            <p className="capitalize-sentence max-w-[75%] truncate text-xs text-foreground">
              {t('billing.subscriptionPlan.billingCycle.current', {
                start: plan.stripeCurrentPeriodStart ? formatStripeDate(plan.stripeCurrentPeriodStart) : '',
                end: plan.stripeCurrentPeriodEnd ? formatStripeDate(plan.stripeCurrentPeriodEnd) : ''
              })}
            </p>
            <p className="text-xs text-muted-foreground">
              {t('billing.subscriptionPlan.billingCycle.daysRemaining', { days: daysToCycleEnd })}
            </p>
          </div>
