...
export function BusinessHoursCard({
  ...
  const onSubmit: SubmitHandler<UpdateBusinessHoursSchema> = async (values) => {
    ...
    if (!result?.serverError && !result?.validationErrors) {
      toast.success(t('general.businessHours.messages.updateSuccess'));
    } else {
      toast.error(t('general.businessHours.messages.updateError'));
    }
  };
  return (
    <FormProvider {...methods}>
      <Card {...other}>
        ...
        <CardFooter className="flex w-full justify-end pt-6">
          ...
          >
{t('general.businessHours.actions.save')}
          </Button>
        </CardFooter>
      </Card>
    </FormProvider>
...
function WorkDay({ name, index, dayOfWeek }: WorkDayProps): React.JSX.Element {
  ...
  const labelFull = React.useMemo(
    () => t(`general.businessHours.fields.${dayOfWeek.toLowerCase()}`),
    [dayOfWeek, t]
  );
      ...
      <div className="col-span-5 sm:col-span-4">
        {hasTimeSlots ? (
          <div className="flex flex-row justify-between">
            ...
            <div className="flex h-9 flex-row items-center">
              <Tooltip delayDuration={0}>
                ...
                <TooltipContent>{t('general.businessHours.actions.addTimeSlot', { defaultValue: 'New time slot' })}</TooltipContent>
              </Tooltip>
              <CopyTimesMenu
                name={name}
          ...
          <p className="flex h-9 items-center pl-2 text-sm text-muted-foreground">
            {t('general.businessHours.fields.closed')}
          </p>
        )}
      </div>
    </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <Tooltip delayDuration={0}>
            ...
            <TooltipContent>{t('general.businessHours.actions.removeTimeSlot', { defaultValue: 'Remove time slot' })}</TooltipContent>
          </Tooltip>
        </div>
        {!isIntervalValid && (
          <FormDescription className="text-destructive">
            {t('general.businessHours.validation.endAfterStart', { defaultValue: 'End time should be after start time.' })}
          </FormDescription>
        )}
        {isIntervalValid && isOverlapping && (
          <FormDescription className="text-destructive">
            {t('general.businessHours.validation.timesOverlap', { defaultValue: 'Times overlap with another set of times.' })}
          </FormDescription>
        )}
      </div>
    );
    ...
    return (
      ...
      >
        <Tooltip delayDuration={0}>
          ...
          <TooltipContent>{t('general.businessHours.actions.copyTimes', { defaultValue: 'Copy times' })}</TooltipContent>
        </Tooltip>
        <PopoverContent className="max-w-[192px] p-2">
          <p className="px-4 py-2 text-sm text-muted-foreground">
            {t('general.businessHours.actions.copyTimesTo', { defaultValue: 'Copy times to...' })}
          </p>
          ...
          <div className="flex flex-row items-center gap-2 p-2 pt-4">
            <PopoverClose>
              <Button
                ...
                className="w-full"
              >
                {t('general.businessHours.actions.cancel', { defaultValue: 'Cancel' })}
              </Button>
            </PopoverClose>
            <PopoverClose>
              <Button
              ...
              >
                {t('general.businessHours.actions.apply', { defaultValue: 'Apply' })}
              </Button>
            </PopoverClose>
          </div>
        </PopoverContent>
