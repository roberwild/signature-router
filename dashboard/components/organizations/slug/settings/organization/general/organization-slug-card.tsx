...
export function OrganizationSlugCard({
  ...
  const onSubmit: SubmitHandler<UpdateOrganizationSlugSchema> = async (
      ...
      if (isValid) {
        ...
        if (!result?.data?.isAvailable) {
          methods.setError('slug', {
            type: 'validate',
            message: t('general.organizationSlug.messages.slugTaken')
          });
          return;
        }
      }
    ...
    if (result?.serverError || result?.validationErrors) {
      toast.error(t('general.organizationSlug.messages.changeError'));
    }
  };
  return (
    <FormProvider {...methods}>
      <Card {...other}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex w-full flex-col">
                  <FormLabel required>{t('general.organizationSlug.fields.slug')}</FormLabel>
                  <FormControl>
                    <Input
                      type="text"
                      maxLength={255}
        ...
        <CardFooter className="flex w-full justify-end pt-6">
          ...
          >
{t('general.organizationSlug.actions.change')}
          </Button>
        </CardFooter>
      </Card>
    </FormProvider>
...
function useShowSlugUpdatedOnQueryParam(): void {
  ...
  React.useEffect(() => {
    if (updated === 'true' && !hasShownToast.current) {
      toast.success(t('general.organizationSlug.messages.changeSuccess'));
