...
export function OrganizationLogoCard({
  ...
  const handleDrop = async (files: File[]): Promise<void> => {
    if (files && files.length > 0) {
      ...
      if (file.size > MAX_IMAGE_SIZE) {
        toast.error(
          t('general.organizationLogo.messages.sizeError', { size: MAX_IMAGE_SIZE / 1000000 })
        );
      } else {
  ...
  const onSubmit: SubmitHandler<UpdateOrganizationLogoSchema> = async (
    ...
    if (!result?.serverError && !result?.validationErrors) {
      toast.success(t('general.organizationLogo.messages.uploadSuccess'));
    } else {
      toast.error(t('general.organizationLogo.messages.uploadError'));
    }
  };
  return (
            ...
            <div className="relative">
              ...
              {!!logo && (
                <Tooltip>
                  ...
                  <TooltipContent side="right">{t('general.organizationLogo.actions.remove')}</TooltipContent>
                </Tooltip>
              )}
            </div>
            <div className="flex flex-col space-y-1">
              <span className="text-sm">{t('general.organizationLogo.actions.upload')}</span>
