...
interface OrganizationListTranslations {
  title?: string;
  description?: string;
  search?: {
    placeholder?: string;
  ...
  return (
    <Card className="border-none shadow-none">
      <CardHeader className="text-center">
        <CardTitle>{translations?.title || 'Organizations'}</CardTitle>
        <CardDescription className="hidden sm:block">
          {translations?.description || 'Jump into an existing organization or add a new one.'}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex items-center space-x-4">
          <InputSearch
            placeholder={translations?.search?.placeholder || 'Search...'}
            value={searchQuery}
            onChange={handleSearchQueryChange}
          />
          <Link
            href={`/${locale}/onboarding/organization`}
          ...
          >
            ...
            <span className="hidden sm:inline">{translations?.addOrganization?.full || 'Add organization'}</span>
            <span className="inline sm:hidden">{translations?.addOrganization?.short || 'Add'}</span>
          </Link>
        </div>
        {filteredOrganizations.length === 0 ? (
          <EmptyState
            ...
            title={translations?.emptyState?.title || 'No organization found'}
            description={
              searchQuery
                ? (translations?.emptyState?.descriptionWithSearch || 'Adjust your search query to show more.')
                : (translations?.emptyState?.descriptionWithoutSearch || 'Add your first organization to get started.')
            }
          />
        ) : (
