...
// Interfaces for parsed data from localStorage
...
interface ServicesChatbotStandaloneProps {
...
export function ServicesChatbotStandalone({ 
  organizationId, 
  organizationSlug,
  organizationName,
  userId,
  userName,
  userEmail
}: ServicesChatbotStandaloneProps) {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        const convs = parsed.map((c: ParsedConversation) => ({
          ...c,
          messages: c.messages.map((m: ParsedMessage) => ({
            ...m,
        ...
        // Load the first conversation or create new
        if (convs.length > 0) {
    ...
    setConversations(prev => {
      const updated = prev.map(c => {
        if (c.id === currentConversationId) {
          ...
          let title = c.title;
          if (title === 'Nueva conversación' && updatedMessages.length > 1) {
            ...
            <div className="py-2">
              {conversations.map(conv => (
                <div
                  key={conv.id}
                  className={cn(
                    "group flex items-center gap-2 px-3 py-1.5 mx-2 rounded-md hover:bg-accent/50 cursor-pointer transition-colors",
