  ...
  const handleDrop = async (files: File[]): Promise<void> => {
    if (files && files.length > 0) {
      ...
      if (file.size > MAX_IMAGE_SIZE) {
        toast.error(
          t('messages.imageSizeError').replace('{{size}}', String(MAX_IMAGE_SIZE / 1000000))
        );
      } else {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        <FormLabel>{t('steps.organization.fields.logo.label')}</FormLabel>
          ...
          <div className="relative">
            ...
            {!!logo && (
              <Tooltip>
                ...
                <TooltipContent side="right">{t('steps.organization.fields.logo.removeTooltip')}</TooltipContent>
              </Tooltip>
            )}
          </div>
          <div className="flex flex-col space-y-1">
            <span className="text-sm">{t('steps.organization.fields.logo.uploadText')}</span>
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem className="flex w-full flex-col">
            <FormLabel required>{t('steps.organization.fields.name.label')}</FormLabel>
            <FormControl>
              <Input
                type="text"
                maxLength={64}
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem className="flex w-full flex-col">
            <FormLabel required>{t('steps.organization.fields.slug.label')}</FormLabel>
            <FormControl>
              <Input
                type="text"
                maxLength={255}
        ...
        render={({ field }) => (
          <FormItem className="flex flex-row items-center justify-between">
            <div className="space-y-0.5">
              <FormLabel>{t('steps.organization.fields.exampleData.label')}</FormLabel>
              <FormDescription>
                {t('steps.organization.fields.exampleData.description')}
              </FormDescription>
            </div>
            <FormControl>
              <Switch
        ...
        onClick={async () => {
          ...
          if (!result?.data?.isAvailable) {
            methods.setError('organizationStep.slug', {
              type: 'validate',
              message: t('messages.slugUnavailable')
            });
            return;
          }
          handleNext();
