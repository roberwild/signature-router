...
public class SecurityAuditController {
     ...
     * 
     * <p>Scans all routing rules in the database and validates their SpEL expressions
     * using the current security rules. Identifies rules with potentially dangerous
     * expressions that should be reviewed manually.</p>
     * 
    ...
    @ApiResponses({
        @ApiResponse(
            ...
            content = @Content(
                ...
                schema = @Schema(implementation = SecurityAuditReport.class)
            )
        ),
        @ApiResponse(
            responseCode = "401",
    ...
    public ResponseEntity<SecurityAuditReport> auditRoutingRules() {
        ...
        List<AuditResult> auditResults = allRules.stream()
            .map(rule -> {
                com.bank.signature.domain.service.SpelValidatorService.ValidationResult validationResult =
                    spelValidatorService.validateWithResult(rule.getCondition());
