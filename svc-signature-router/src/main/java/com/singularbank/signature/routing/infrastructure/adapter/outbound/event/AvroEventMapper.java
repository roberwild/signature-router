...
import com.singularbank.signature.routing.events.avro.CircuitBreakerClosedPayload;
import com.singularbank.signature.routing.events.avro.CircuitBreakerOpenedPayload;
import com.singularbank.signature.routing.events.avro.SignatureAbortedPayload;
import com.singularbank.signature.routing.events.avro.SignatureCompletedPayload;
 ...
 *
 * <p>
 * <b>Story 5.5 - Event Serialization:</b> This mapper ensures that domain
 * events
 * are properly serialized to Avro format before being published to Kafka via
 * Debezium.
 * </p>
 *
 * @since Story 5.5
...
public class AvroEventMapper {
    ...
    public Object toAvro(DomainEvent domainEvent) {
        return switch (domainEvent) {
            case com.singularbank.signature.routing.domain.event.SignatureCompletedEvent event -> toAvro(event);
            case com.singularbank.signature.routing.domain.event.SignatureAbortedEvent event -> toAvro(event);
            case com.singularbank.signature.routing.domain.event.CircuitBreakerOpenedEvent event -> toAvro(event);
            case com.singularbank.signature.routing.domain.event.CircuitBreakerClosedEvent event -> toAvro(event);
            case com.singularbank.signature.routing.domain.event.CircuitBreakerHalfOpenEvent event -> toAvro(event);
            case com.singularbank.signature.routing.domain.event.CircuitBreakerFailedRecoveryEvent event -> toAvro(event);
            case com.singularbank.signature.routing.domain.event.CircuitBreakerResetEvent event -> toAvro(event);
     ...
     */
    public com.singularbank.signature.routing.events.avro.SignatureCompletedEvent toAvro(
            com.singularbank.signature.routing.domain.event.SignatureCompletedEvent domain) {
        ...
        return com.singularbank.signature.routing.events.avro.SignatureCompletedEvent.newBuilder()
                ...
                .setPayload(SignatureCompletedPayload.newBuilder()
                        ...
                        .setStatus("COMPLETED")
                        .setCompletedAt(Instant.now().toEpochMilli()) // completedAt field removed from DB schema
     ...
     */
    public com.singularbank.signature.routing.events.avro.SignatureAbortedEvent toAvro(
            com.singularbank.signature.routing.domain.event.SignatureAbortedEvent domain) {

        return com.singularbank.signature.routing.events.avro.SignatureAbortedEvent.newBuilder()
                .setEventId(domain.getEventId().toString())
     ...
     */
    public com.singularbank.signature.routing.events.avro.CircuitBreakerOpenedEvent toAvro(
            com.singularbank.signature.routing.domain.event.CircuitBreakerOpenedEvent domain) {

        return com.singularbank.signature.routing.events.avro.CircuitBreakerOpenedEvent.newBuilder()
                .setEventId(domain.getEventId().toString())
     ...
     */
    public com.singularbank.signature.routing.events.avro.CircuitBreakerClosedEvent toAvro(
            com.singularbank.signature.routing.domain.event.CircuitBreakerClosedEvent domain) {

        return com.singularbank.signature.routing.events.avro.CircuitBreakerClosedEvent.newBuilder()
                .setEventId(domain.getEventId().toString())
    ...
    /**
     * Maps CircuitBreakerHalfOpenEvent (domain) → CircuitBreakerOpenedEvent (Avro
     * with state HALF_OPEN).
     * Note: We reuse CircuitBreakerOpenedEvent schema for HALF_OPEN state.
     */
    public com.singularbank.signature.routing.events.avro.CircuitBreakerOpenedEvent toAvro(
            com.singularbank.signature.routing.domain.event.CircuitBreakerHalfOpenEvent domain) {
    ...
    /**
     * Maps CircuitBreakerFailedRecoveryEvent (domain) → CircuitBreakerOpenedEvent
     * (Avro with state OPEN).
     */
    public com.singularbank.signature.routing.events.avro.CircuitBreakerOpenedEvent toAvro(
            com.singularbank.signature.routing.domain.event.CircuitBreakerFailedRecoveryEvent domain) {
     ...
     */
    public com.singularbank.signature.routing.events.avro.CircuitBreakerClosedEvent toAvro(
            com.singularbank.signature.routing.domain.event.CircuitBreakerResetEvent domain) {
