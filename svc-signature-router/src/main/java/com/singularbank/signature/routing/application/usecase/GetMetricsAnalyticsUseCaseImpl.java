...
import com.singularbank.signature.routing.application.dto.response.MetricsAnalyticsResponse.ChallengeCompletionMetrics;
import com.singularbank.signature.routing.application.dto.response.MetricsAnalyticsResponse.ErrorRateMetrics;
import com.singularbank.signature.routing.application.dto.response.MetricsAnalyticsResponse.LatencyMetrics;
import com.singularbank.signature.routing.application.dto.response.MetricsAnalyticsResponse.SignatureDurationMetrics;
import com.singularbank.signature.routing.application.dto.response.MetricsAnalyticsResponse.ThroughputMetrics;
 ...
 * 
 * Provides advanced metrics analytics with latency, throughput, and error
 * rates.
...
public class GetMetricsAnalyticsUseCaseImpl implements GetMetricsAnalyticsUseCase {
        ...
        for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
            ...
            if (dayTotal > 0) {
                long dayFailed = channel != null
                        ? 0L /* TODO: SignatureRequestEntity no tiene campo 'channel' */
                        : signatureRequestRepository.countByStatusAndCreatedAtBetween(SignatureStatus.FAILED, dayStart,
                                dayEnd);

                long dayExpired = channel != null
                        ? 0L /* TODO: SignatureRequestEntity no tiene campo 'channel' */
                        : signatureRequestRepository.countByStatusAndCreatedAtBetween(SignatureStatus.EXPIRED, dayStart,
                                dayEnd);
    ...
    private SignatureDurationMetrics computeSignatureDurationMetrics(Instant from, Instant to, String range) {
        ...
        // Calculate by channel (placeholder - would need channel info on
        // SignatureRequest)
    ...
    private ChallengeCompletionMetrics computeChallengeCompletionMetrics(Instant from, Instant to) {
        ...
        List<SignatureRequest> requestsWithSentChallenges = signatureRequestRepository
                .findWithSentChallengesBetween(from, to);
        List<SignatureRequest> requestsWithCompletedChallenges = signatureRequestRepository
                .findWithCompletedChallengesBetween(from, to);

        // Extract all challenges
        List<com.singularbank.signature.routing.domain.model.entity.SignatureChallenge> sentChallenges = requestsWithSentChallenges
                .stream()
                .flatMap(sr -> sr.getChallenges().stream())
                .filter(c -> c.getSentAt() != null && !c.getSentAt().isBefore(from) && c.getSentAt().isBefore(to))
                .collect(Collectors.toList());

        List<com.singularbank.signature.routing.domain.model.entity.SignatureChallenge> completedChallenges = requestsWithCompletedChallenges
                .stream()
                .flatMap(sr -> sr.getChallenges().stream())
                .filter(c -> c.getCompletedAt() != null && !c.getCompletedAt().isBefore(from)
                        && c.getCompletedAt().isBefore(to))
                .collect(Collectors.toList());
        ...
        Map<String, List<com.singularbank.signature.routing.domain.model.entity.SignatureChallenge>> sentByChannel = sentChallenges
                .stream()
                .filter(c -> c.getChannelType() != null)
                .collect(Collectors.groupingBy(c -> c.getChannelType().name()));

        Map<String, List<com.singularbank.signature.routing.domain.model.entity.SignatureChallenge>> completedByChannel = completedChallenges
                .stream()
                .filter(c -> c.getChannelType() != null)
                .collect(Collectors.groupingBy(c -> c.getChannelType().name()));

        for (String channelName : sentByChannel.keySet()) {
            List<com.singularbank.signature.routing.domain.model.entity.SignatureChallenge> channelSent = sentByChannel
                    .get(channelName);
            List<com.singularbank.signature.routing.domain.model.entity.SignatureChallenge> channelCompleted = completedByChannel
                    .getOrDefault(channelName, new ArrayList<>());
        ...
        for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
            ...
            List<com.singularbank.signature.routing.domain.model.entity.SignatureChallenge> daySent = sentChallenges.stream()
                    .filter(c -> c.getSentAt() != null && !c.getSentAt().isBefore(dayStart)
                            && c.getSentAt().isBefore(dayEnd))
                    .collect(Collectors.toList());

            List<com.singularbank.signature.routing.domain.model.entity.SignatureChallenge> dayCompleted = completedChallenges.stream()
                    .filter(c -> c.getCompletedAt() != null && !c.getCompletedAt().isBefore(dayStart)
                            && c.getCompletedAt().isBefore(dayEnd))
                    .collect(Collectors.toList());
            ...
            double dayCompletionRate = daySent.isEmpty() ? 0.0 : (dayCompleted.size() * 100.0) / daySent.size();
