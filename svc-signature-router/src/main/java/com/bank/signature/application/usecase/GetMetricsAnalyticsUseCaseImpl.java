...
import com.bank.signature.application.dto.response.MetricsAnalyticsResponse.ChallengeCompletionMetrics;
import com.bank.signature.application.dto.response.MetricsAnalyticsResponse.ErrorRateMetrics;
import com.bank.signature.application.dto.response.MetricsAnalyticsResponse.LatencyMetrics;
import com.bank.signature.application.dto.response.MetricsAnalyticsResponse.SignatureDurationMetrics;
import com.bank.signature.application.dto.response.MetricsAnalyticsResponse.ThroughputMetrics;
...
public class GetMetricsAnalyticsUseCaseImpl implements GetMetricsAnalyticsUseCase {
    ...
    private ChallengeCompletionMetrics computeChallengeCompletionMetrics(Instant from, Instant to) {
        ...
        List<com.bank.signature.domain.model.entity.SignatureChallenge> sentChallenges = requestsWithSentChallenges
                .stream()
                .flatMap(sr -> sr.getChallenges().stream())
                .filter(c -> c.getSentAt() != null && !c.getSentAt().isBefore(from) && c.getSentAt().isBefore(to))
                .collect(Collectors.toList());

        List<com.bank.signature.domain.model.entity.SignatureChallenge> completedChallenges = requestsWithCompletedChallenges
                .stream()
                .flatMap(sr -> sr.getChallenges().stream())
        ...
        Map<String, List<com.bank.signature.domain.model.entity.SignatureChallenge>> sentByChannel = sentChallenges
                .stream()
                .filter(c -> c.getChannelType() != null)
                .collect(Collectors.groupingBy(c -> c.getChannelType().name()));

        Map<String, List<com.bank.signature.domain.model.entity.SignatureChallenge>> completedByChannel = completedChallenges
                .stream()
                .filter(c -> c.getChannelType() != null)
                .collect(Collectors.groupingBy(c -> c.getChannelType().name()));

        for (String channelName : sentByChannel.keySet()) {
            List<com.bank.signature.domain.model.entity.SignatureChallenge> channelSent = sentByChannel
                    .get(channelName);
            List<com.bank.signature.domain.model.entity.SignatureChallenge> channelCompleted = completedByChannel
