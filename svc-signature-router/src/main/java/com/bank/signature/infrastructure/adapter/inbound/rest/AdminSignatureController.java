 ...
 * 
 * <p><b>Security:</b> ADMIN, SUPPORT, OPERATOR, or VIEWER role required.</p>
 * 
 * @since Story 2.12
 */
@RestController
...
public class AdminSignatureController {
    ...
    @ApiResponses(value = {
        @ApiResponse(
            ...
            content = @Content(
                ...
                schema = @Schema(implementation = AdminSignatureListResponse.class),
                examples = @ExampleObject(
                    name = "Signature List Example",
                    value = """
                        {
                          "content": [
                            {
    ...
    public ResponseEntity<AdminSignatureListResponse> listSignatures(
        ...
        @RequestParam(required = false) SignatureStatus status,
        
        @Parameter(description = "Filter by channel (optional)")
        @RequestParam(required = false) Channel channel,
        
        @Parameter(description = "Start date filter (ISO 8601, optional)")
        @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Instant dateFrom,
        
        @Parameter(description = "End date filter (ISO 8601, optional)")
