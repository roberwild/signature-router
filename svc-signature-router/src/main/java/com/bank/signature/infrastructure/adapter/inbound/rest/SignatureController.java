...
public class SignatureController {
     ...
     * @param idempotencyKey Idempotency key for duplicate prevention (optional for Story 2.1)
    ...
    @ApiResponses(value = {
        @ApiResponse(
            ...
            headers = @Header(
                name = "Location",
                description = "URI of the created signature request",
                schema = @Schema(type = "string", example = "/api/v1/signatures/01933e5d-7c2f-7890-a1b2-c3d4e5f60001")
            ),
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = SignatureResponseDto.class)
            )
        ),
        @ApiResponse(
            responseCode = "400",
        ...
        @ApiResponse(
            responseCode = "422",
            description = "Business rule violation (domain exception)",
            content = @Content(mediaType = "application/json")
        )
    })
    public ResponseEntity<SignatureResponseDto> createSignatureRequest(
        @Parameter(description = "Signature request details", required = true)
    ...
    private ResponseEntity<SignatureResponseDto> handleDegradedModeRequest(CreateSignatureRequestDto request) {
        ...
        // Note: This is a workaround. Ideally, StartSignatureUseCase should handle degraded mode.
    ...
    @ApiResponses(value = {
        @ApiResponse(
            ...
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = SignatureRequestDetailDto.class)
            )
        ),
        @ApiResponse(
            responseCode = "404",
    ...
    /**
     * Completes a signature request by validating the challenge code provided by the user.
     * Story 2.11: Signature Completion (User Response)
     * 
     * <p><b>Story 2.11 Acceptance Criteria:</b></p>
     * <ul>
    ...
    @ApiResponses(value = {
        @ApiResponse(
            ...
            content = @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = SignatureCompletionResponseDto.class)
            )
        ),
        @ApiResponse(
            responseCode = "400",
            description = "Invalid challenge code or challenge in invalid state",
            content = @Content(mediaType = "application/json")
        ),
        @ApiResponse(
            responseCode = "404",
            description = "Signature request or challenge not found",
            content = @Content(mediaType = "application/json")
        ),
        @ApiResponse(
            responseCode = "401",
    ...
    public ResponseEntity<SignatureCompletionResponseDto> completeSignature(
        ...
        @Parameter(
            description = "Completion request containing challengeId and OTP code",
            required = true
        )
        @Valid @RequestBody CompleteSignatureDto request
    ) {
